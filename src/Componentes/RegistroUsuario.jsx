...
const RegistroUsuario = ({ idUsuario }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                setToastMsg(idUsuario ? "Usuario actualizado correctamente." : "Usuario creado correctamente.");
                setShowToast(true);
                setExito(true);
                limpiarCampos();
    ...
    return (
        <form onSubmit={handleSubmit} className="max-w-md mx-auto bg-white p-6 rounded shadow space-y-4">
            <h2 className="text-2xl font-bold mb-4">{idUsuario ? "Editar Usuario" : "Registro de Usuario"}</h2>
            <input
                name="nombre"
                placeholder="Nombre"
                value={form.nombre}
            ...
            <button
                type="submit"
                className="w-100 btn btn-success fw-bold py-2"
            >
                {idUsuario ? "Actualizar Usuario" : "Registrar Usuario"}
            </button>
            {mensaje && (
                <div className={`mt-2 text-center ${exito ? "text-green-600" : "text-red-600"}`}>
                    {mensaje}
                </div>
            )}
        </form>
